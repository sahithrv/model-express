...
The optional raw execution-event v2 stream is exposed at `GET /projects/:id/events/stream/v2` only when `MODEL_EXPRESS_ACTIVITY_STREAM_V2_ENABLED=true`. Under the same flag, `HEAD` on that path validates project and cursor compatibility without opening a stream and returns `204` when the client can connect. The flag defaults to false, so rollback disables both routes without removing or repairing cursor data. The existing execution-event and synthesized activity endpoints remain unchanged.
...
Mission Control gates the incremental client with `MODEL_EXPRESS_MISSION_CONTROL_LIVE_V2_ENABLED`, `MODEL_EXPRESS_MISSION_CONTROL_LIVE_V2_SHADOW`, and `MODEL_EXPRESS_MISSION_CONTROL_LIVE_V2_ROLLBACK`. It keeps legacy polling and the v1 synthesized activity stream active until runtime flags are loaded. Shadow mode consumes and compares v2 state without changing visible data. Primary mode disables broad polling only after the live-state snapshot is installed and the authenticated v2 stream is connected, supported, cursor-consistent, and healthy; typed events then refresh only their allowlisted resources. Compact snapshot progress changes also target metrics, preserving old-worker metric updates that predate per-boundary event reporting, and transient targeted failures receive one bounded follow-up. Unsupported endpoints, disconnects, bounded cursor-recovery exhaustion, or rollback immediately restore conservative ten/thirty-second legacy polling. Manual refresh and on-demand detail loading remain available in every mode.
