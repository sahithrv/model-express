...
When an ONNX champion export is produced, the worker also attaches Portable Inference Bundle V1 as optional portability material. The bundle is a ZIP artifact recorded in the existing export manifest with `format: portable_inference_bundle`; it does not replace the requested model artifact, and `champion_exports.artifact_uri` remains the primary `.onnx` URI for ONNX exports. The ZIP contains `model.onnx`, any ONNX external-data sidecars, a runtime-neutral `manifest.json`, Python and Node ONNX Runtime reference examples, `requirements.txt`, `README.md`, and `parity/expected_outputs.json` from export self-test data when available. The bundle contract is standalone: it must not require `model_express_runtime`, Mission Control, the backend API, Postgres, worker polling, or any Model Express package to run inference. The included examples are reference code only; consumers should read `manifest.json` and implement the declared preprocessing, ONNX inference, and postprocessing contract in their chosen runtime.
