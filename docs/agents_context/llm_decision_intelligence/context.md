...
Training Monitor prompt input is compacted into `training_monitor_run_evaluation_cards_v1`. The backend still stores the full run summary, evaluation payload, epoch metrics, plan, and job config, but the LLM sees only compact cards: `run_summary_card`, `training_dynamics_card`, `per_class_confusion_card`, `deployment_model_profile_card`, `objective_fit_card`, `memory_summary`, and `prompt_budget`. The monitor context omits full epoch history, full model/profile dumps, raw job config, raw plan experiments, raw evaluation payloads, and memory payloads. `prompt_budget` records caps plus approximate context/input bytes and a rough token estimate for invocation telemetry.
...
Backend stop criteria can override an LLM `ADD_EXPERIMENTS` recommendation and persist `SELECT_CHAMPION`. This happens after repeated no-improvement follow-up rounds and when the current champion is within the minimum meaningful improvement threshold of a bounded metric ceiling, such as accuracy or macro-F1 near 1.0. The LLM should treat near-ceiling performance as a strong stop/select signal, not as an invitation to spend credits chasing impossible gains. Persisted champion selection is terminal for autonomous planning: if a project has a project champion or any persisted `SELECT_CHAMPION` decision, the backend skips new planner calls and stale/new autonomous `ADD_EXPERIMENTS` paths must not create follow-up plans or jobs; blocked paths record `backend_stop_guard: champion_selected_guard`. Continuing after champion selection requires explicit user reopen via `POST /projects/:id/experimentation/reopen`, which records `REOPEN_EXPERIMENTATION` and `EXPERIMENTATION_REOPENED`.
...
Mechanisms are now first-class. Each candidate hypothesis must include `mechanism`, `intervention`, `evidence_used`, and `expected_effect`; direct `proposed_experiments` must include a matching `proposal_mechanisms` sidecar with `experiment_index`. Accepted sidecar metadata is validated by the backend and copied onto stored `PlannedExperiment`s. Treat model family as a parameter inside a mechanism, not as the mechanism itself.
...
Structured augmentation is now explicit in planner-facing configs. Use `augmentation_policy_config` only for bounded policies (`basic`, `randaugment`, `trivialaugment`, `trivialaugmentwide`, `autoaugment`, `mixup`, `cutmix`) with backend/worker caps. MixUp/CutMix are training-batch-only mixed-sample mechanisms and should be proposed only when overfitting, small data, boundary ambiguity, or calibration evidence supports them.
...
Strategy scorecards are structured outcome memory separate from raw agent memory. They track strategy type, planning mode, mechanism, intervention, diagnosis triggers, evidence used, expected effect, dataset traits, objective profile, proposed changes, expected delta, actual delta, confidence before/after, cost, runtime, outcome, lesson, and tags. Future planner prompts receive these as compact `strategy_lessons`; scorecard-derived lessons expose mechanism, intervention, diagnosis triggers, expected delta, and actual delta when those fields are available. The planner should prefer `improved_champion` lessons and avoid similar failed/no-improvement mechanism lessons.

Planner context cards V2 are first-class prompt contracts. `training_dynamics_card` summarizes best/final metric, loss gap, recent metric slope, plateau/instability scores, and whether more epochs are justified. `per_class_error_card` summarizes worst recall/F1 classes, top confusion pairs, accuracy-vs-macro-F1 gap, and whether class balancing/minority targeting is active. `deployment_card` summarizes latency/throughput/size fields, objective weights, champion/challenger tradeoffs, and whether quality gains can survive deployment penalties. `mechanism_coverage_card` summarizes tried, blocked, and eligible mechanisms, best result by mechanism, failed mechanism lessons, and shallow-repeat warnings. `label_quality_card` summarizes suspicious classes, asymmetric confusions, high-confidence error counts, and audit-only label-quality signals.
...
Recent work added richer dataset profiles, preprocessing-aware planned experiment fields, backend validation, worker-visible preprocessing config, deterministic diagnosis, planning modes, candidate ranking, rejected options, first-class mechanism scorecards, report-only label-quality audits, and Mission Control visibility for agent reasoning.
