...
type Store interface {
	...
	UpsertRemoteTrainingSession(session runs.RemoteTrainingSession) (runs.RemoteTrainingSession, error)
	GetRemoteTrainingSession(id string) (runs.RemoteTrainingSession, error)
	CloseRemoteTrainingSession(id string, status string) (runs.RemoteTrainingSession, error)
	...
	CreateChampionFeedback(feedback runs.ChampionFeedbackCreate) (runs.ChampionFeedback, error)
	...
	GetMemoryEmbeddingSourceState(projectID string, sourceTable string, sourceID string, embeddingModel string) (memory.MemoryEmbeddingSourceState, error)
	CreateMemoryEmbeddingUsageEvent(event memory.MemoryEmbeddingUsageEvent) (memory.MemoryEmbeddingUsageEvent, error)
	ListProjectMemoryEmbeddingUsageEvents(projectID string, limit int) ([]memory.MemoryEmbeddingUsageEvent, error)
	CountProjectMemoryEmbeddingUsageEvents(projectID string, purpose string, since time.Time) (int, error)
	GetMemoryRetrievalQueryCache(projectID string, datasetID string, purpose string, embeddingModel string, embeddingDimensions int, normalizedQueryHash string) (memory.MemoryRetrievalQueryCacheRecord, error)
	UpsertMemoryRetrievalQueryCache(record memory.MemoryRetrievalQueryCacheRecord) (memory.MemoryRetrievalQueryCacheRecord, error)
	ListUnembeddedMemorySources(projectID string, embeddingModel string, embeddingDimensions int, limit int) ([]memory.EmbeddableMemoryCard, error)
