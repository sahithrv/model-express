...
from worker.training.modal_provider import run_modal_dataset_materialization


MODAL_DISPATCHER_TEMPLATES = ["train_experiment", "profile_dataset"]
MODAL_DISPATCHER_UNSPECIFIED_PROVIDER_TEMPLATES = ["profile_dataset"]
...
class ModalDispatcher:
    def __init__(
        ...
        *,
        ...
        self._executor = ThreadPoolExecutor(max_workers=max(1, self.slot_count), thread_name_prefix="modal-dispatcher")
...
def modal_dispatcher_slot_count() -> int:
    return _positive_int_env("MODEL_EXPRESS_MODAL_DISPATCHER_SLOTS", DEFAULT_DISPATCHER_SLOTS, minimum=1, maximum=8)
