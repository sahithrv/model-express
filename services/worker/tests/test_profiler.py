...
class DatasetProfilerTests(unittest.TestCase):
    ...
    def test_profile_unwraps_single_wrapper_and_common_image_root(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(profile["metadata_summary"]["bbox_available"])
            self.assertFalse(profile["metadata_summary"]["yolo_available"])
            self.assertFalse(profile["metadata_summary"]["yolo_format"])
            self.assertEqual(profile["metadata_summary"]["bbox_count"], 2)
            self.assertEqual(profile["metadata_summary"]["yolo_config_count"], 0)
            self.assertEqual(profile["metadata_summary"]["yolo_label_file_count"], 0)
            self.assertEqual(profile["bbox_count"], 2)
            self.assertFalse(profile["yolo_available"])
    ...
    def test_profile_detects_yolo_object_detection_layout(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(profile["metadata_summary"]["yolo_format"])
            self.assertEqual(profile["metadata_summary"]["yolo_config_count"], 1)
            self.assertEqual(profile["metadata_summary"]["yolo_label_file_count"], 2)
